...
1. **Measured artifact provenance leads the spoken USP** - auto-captured tools, skills, costs, and session usage: "what people measurably run, not what they claim." Single-author editorial (aicoolies) and anonymous atom directories (skills.sh) cannot fake this.
2. **The living person-centric stack page is the surface people live on** - real-time stats give visitors a reason to return and deliver direct value to the creator.
...
Consultant marketplace: **parked** (P3), as a measured "available for hire" affordance.
