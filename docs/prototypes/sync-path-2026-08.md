...
Rejected: **aliveness** ("a hand-built stack goes stale"). It scolds the reader
for the thing they already did.

Cancel is the **safety line under the ask**, not the ask itself. The page never
offers "just look" as the headline — that would disagree with a broadcast whose
call to action is to sync.
...
machine, and nothing else.** Both of the page's hard sentences fall out of it.

- It is why raw data never leaves the machine.
- It is why chat apps cannot be measured.

So the page states the boundary once and never apologizes for anything.

**The page does not name Claude or ChatGPT.** Locked by the owner. The boundary
sentence carries the fact, and naming the two chat apps invites a comparison the
page does not want to hold. This answers ticket question 2: the page says it by
not saying it.
...
`searched` line in §4 reports what the CLI **looked for** — a claim about the
CLI, not about the person's behavior.
...
It is the only place the page tells the reader that cancelling still pays.
...
the blocks below. No found/not-found marks — that is the compatibility matrix
arriving through the back door.

Why it earns its place: without it, a person who genuinely runs opencode but
whose install path the scan misses sees the same silence as a person who does
not run opencode at all. Silence protects #40 and hides the bug.

### 4.2 The per-harness header becomes unconditional

**The non-obvious consequence, and the one real regression risk in #131.**
...
the single-harness shape and will need updating with intent, not by snapshot
blessing.
...
- **`HARNESS` on the stack page.** Named above, deliberately untouched.
