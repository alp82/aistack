...
- **E "Reel"** — insights taking turns on one bold stage (words mode).
...
- **Chip styling took three rounds**: bordered chips read as controls (the
  select owns that look); solid fills failed next (white too heavy on the dark
  canvas, lime fights the lime line). Final: **quiet glass** — translucent
  canvas, no border, small lime square marking the live chip.
...
- The animation is not the record: an sr-only sentence carries the canonical
  figures in the first HTML; the animated pair is aria-hidden.
