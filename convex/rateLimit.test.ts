...
test('TC-RL-01: first call for a fresh IP returns allowed:true, remaining:59', async () => {
  ...
  const result = await t.mutation(api.rateLimit.checkApiRateLimit, { key: 'ip:10.0.0.1' })
...
test('TC-RL-02: 60 calls for one IP are all allowed:true, remaining descends 59→0, 60th has remaining:0', async () => {
  ...
  for (let i = 0; i < MAX_REQUESTS; i++) {
    lastResult = await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ip}` })
...
test('TC-RL-03: 61st call same IP within window returns allowed:false, remaining:0, retryAfterSeconds>0', async () => {
  ...
  for (let i = 0; i < MAX_REQUESTS; i++) {
    await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ip}` })
  }

  // 61st call must be rejected
  const result = await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ip}` })
...
test('TC-RL-04: two distinct IPs are independent — exhausting one does not affect the other', async () => {
  ...
  // Exhaust ipA
  for (let i = 0; i < MAX_REQUESTS; i++) {
    await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ipA}` })
  }
  // ipA is now saturated
  const saturated = await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ipA}` })
  expect(saturated.allowed).toBe(false)

  // First call for ipB must still be allowed
  const fresh = await t.mutation(api.rateLimit.checkApiRateLimit, { key: `ip:${ipB}` })
...
test('TC-RL-07: cleanupExpiredRateLimits deletes expired rows and leaves live rows intact', async () => {
  ...
  await t.run(async (ctx: MutationCtx) => {
    const expired = await ctx.db
      .query('apiRateLimits')
      .withIndex('by_key', (q) => q.eq('key', 'ip:expired'))
      .first()
    expect(expired).toBeNull()

    const live = await ctx.db
      .query('apiRateLimits')
      .withIndex('by_key', (q) => q.eq('key', 'ip:live'))
      .first()
