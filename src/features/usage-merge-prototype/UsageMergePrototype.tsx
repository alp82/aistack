...
import type { WindowId } from "@/features/workflow/copy";
...
export function UsageMergePrototype({
	...
	if (!V) {
		throw new Error(
			`PROTOTYPE #303: no variant "${String(variant)}". Known: ${KEYS.join(",")}`,
		);
	}
	return (
		<>
...
export function isVariantKey(v: unknown): v is VariantKey {
	return typeof v === "string" && Object.hasOwn(VARIANTS, v);
}
