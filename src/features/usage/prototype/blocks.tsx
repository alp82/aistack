...
export function Tile({
	...
	return (
		<div
			className={cn(
				"flex min-w-0 flex-col gap-1.5 border border-stroke-subtle bg-bg-panel",
				compact ? "p-2" : "p-4",
			)}
		>
			<span
				className={cn(
					...
					compact ? "text-xl" : big ? "text-4xl" : "text-[28px]",
				)}
			>
				{tile.value}
			</span>
			...
			<span
				className={cn(
					"flex gap-2",
					compact ? "flex-col" : "items-baseline justify-between",
				)}
			>
				<span
					className={cn(
...
export function HarnessPie({
	...
	rows: readonly ShareRow[];
	size?: string;
	/** Pie over the legend instead of beside it (#465, phone width). */
	stacked?: boolean;
}) {
	const R = 20;
	const C = 2 * Math.PI * R;
	...
	return (
		<div
			className={cn(
				"flex items-center",
				stacked ? "flex-col items-stretch gap-5" : "gap-6",
			)}
		>
			<svg
				viewBox="0 0 100 100"
				className={cn(size, "shrink-0 -rotate-90", stacked && "self-center")}
				role="img"
...
export function PlateStrip({
	...
	withheld: number;
	/** The narrowest share that still holds its own name (#465 raises it on a phone). */
	minInside?: number;
}) {
	const steps = [100, 78, 60, 46, 36, 28];
	const paint = (i: number) =>
		`color-mix(in oklab, var(--accent-lime) ${steps[Math.min(i, steps.length - 1)]}%, var(--bg-panel))`;
	const inside = (item: KitItem) => item.share >= minInside;
