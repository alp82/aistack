...
import { type BillingPeriod, formatPriceDisplay } from "@/lib/pricing";
...
export function MainToolCard({ tool }: { tool: ToolData }) {
	...
	const toolPriceDisplay = tool.price.fixed
		? formatPriceDisplay(tool.price.fixed.amount, tool.price.fixed.period)
		: null;
					...
					<div className="flex items-center justify-between mb-1">
						...
						<span className="text-lg font-bold text-white ml-2 flex-shrink-0">
							{tool.priceKind === "sponsored" ? (
								...
								<>
									{toolPriceDisplay
										? `$${toolPriceDisplay.amountText}`
										: "Usage"}
									{toolPriceDisplay && (
										<span className="text-xs text-gray-500 font-normal">
											{toolPriceDisplay.suffix}
										</span>
									)}
								</>
							)}
							...
							return (
								<span
									...
									className={cn(
										...
										catConfig?.bgColor || "bg-gray-700",
										catConfig?.textColor || "text-gray-300",
									)}
								>
...
export function MiscToolCard({ tool, onBundleClick }: MiscToolCardProps) {
	...
	const config2 = firstCat2
		? categoryConfig[firstCat2 as keyof typeof categoryConfig]
		: undefined;
