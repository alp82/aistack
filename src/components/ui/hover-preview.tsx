...
export type HoverPreviewProps = HoverPreviewInlineProps | HoverPreviewWrapperProps;
...
const HoverPreview = (props: HoverPreviewProps) => {
  ...
  const finalX = useTransform(() => {
    ...
    const padding = 16;
    return Math.max(padding, Math.min(x, window.innerWidth - previewWidth - padding));
  });
  ...
  const finalY = useTransform(() => {
    ...
    const padding = 16;
    return Math.max(padding, Math.min(y, window.innerHeight - previewHeight - padding));
  });
  ...
  const renderPreviewContent = () => {
    if (props.mode === "inline") {
      const { targets, imageBorderRadius = "0.75rem", showImageShadow = true } = props;
