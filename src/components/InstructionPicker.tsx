...
export function InstructionPicker({
	...
	const updateInstruction = (
		index: number,
		updates: Partial<InstructionItem>,
	) => {
		const updated = [...value];
		...
		<div className="space-y-2">
			...
			{showAddForm && (
				<div className="space-y-3 border border-stroke-subtle bg-bg-panel p-3">
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								setNewInstruction({
									...newInstruction,
									type: e.target.value as InstructionType,
								})
							}
					...
					{/* URL (for MCP, Plugin) */}
					{(newInstruction.type === "mcp" ||
						newInstruction.type === "plugin") && (
						<div>
							<label className="mb-1 block font-mono text-[10px] uppercase text-fg-muted">
								URL
							</label>
							<input
								...
								onChange={(e) =>
									setNewInstruction({ ...newInstruction, url: e.target.value })
								}
								placeholder="https://..."
								className="w-full border border-stroke-subtle bg-bg-panel px-2 py-1.5 font-mono text-xs text-fg-primary placeholder:text-fg-muted focus:border-accent-lime focus:outline-none"
							/>
